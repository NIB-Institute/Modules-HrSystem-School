...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: t('Trash'), href: '/dashboard/classrooms/trash' },
];

const trashConfig: TrashConfigLocal = {
    entityLabel: t('Classroom'),
    entityLabelPlural: t('Classrooms'),
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head :title="t('Classrooms Trash')" />

        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <!-- Header -->
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold">{{ t('Classrooms Trash') }}</h1>
                    <p class="text-sm text-muted-foreground">
                        {{ t('Manage deleted classrooms - restore or permanently delete') }}
                    </p>
                </div>
                <ButtonGroup>
            ...
            <TrashTable
                ...
                :selectable="true"
                select-key="uuid"
                :empty-message="t('No deleted classrooms found.')"
