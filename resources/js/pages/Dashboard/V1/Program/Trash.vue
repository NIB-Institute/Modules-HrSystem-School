...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: __('Trash'), href: '/dashboard/programs/trash' },
];

const trashConfig: TrashConfigLocal = {
    entityLabel: __('Program'),
    entityLabelPlural: __('Programs'),
...
const handleBulkRestore = () => {
    router.put('/dashboard/programs/trash/bulk-restore', {
...
const handleBulkForceDelete = () => {
    if (confirm(__('Are you sure you want to permanently delete :count item(s)? This action cannot be undone.', { count: selectedUuids.value.length }))) {
        router.delete('/dashboard/programs/trash/bulk-force-delete', {
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head :title="__('Programs Trash')" />

        <div class="flex h-full flex-1 flex-col gap-6 p-6">
            <!-- Header -->
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold">{{ __('Programs Trash') }}</h1>
                    <p class="text-sm text-muted-foreground">
                        {{ __('Manage deleted programs - restore or permanently delete') }}
                    </p>
                </div>
                <ButtonGroup>
            ...
            <TrashTable
                ...
                :selectable="true"
                select-key="uuid"
                :empty-message="__('No deleted programs found.')"
